"""
PTB-XL ECG Arrhythmia Classification - Kaggle Training Script (Phase 1: 500Hz)
====
...
Phase 1 500Hz adaptation by: <you>
"""
...
# Model configuration (Phase 1: 500Hz upgrade)
TARGET_CLASSES = 14
SAMPLING_RATE = 500         # Hz (was 100)
SIGNAL_LENGTH = 5000        # 10 seconds @ 500Hz (was 1000)
...
def extract_primary_label(scp_dict):
    ...
    Strategy (unchanged for compatibility):
    - Use codes with confidence >= 50
    - Priority-based selection following CLASS_MAPPING order
    """
    if not scp_dict:
        return "NORM"
...
def load_ecg_signal(filename_hr, data_dir, target_length=SIGNAL_LENGTH):
    """
    Load ECG signal from WFDB format (.hea + .dat files) at 500Hz.

    Returns:
        numpy array of shape (target_length, 12)
    """
    try:
        import wfdb

        # filename_hr in PTB-XL metadata contains the high-resolution record name
        record_name = str(filename_hr).replace(".hea", "").replace(".dat", "")
        record_path = data_dir / record_name

        # Read WFDB record; PTB-XL has 5000 samples for 10 seconds @ 500Hz
        record = wfdb.rdrecord(str(record_path))

        signal = record.p_signal  # Shape: (samples, 12 leads)

        # Basic safety: pad/truncate to target_length
...
def preprocess_signal(signal, target_length=SIGNAL_LENGTH):
    ...
    2. Pad or truncate to target length (safety, though we aim for 5000)
    """
...
# ====
# 4. MODEL ARCHITECTURE: ResNet1D (adjusted to 5000 length)
# ====
...
def residual_block(x, filters, kernel_size=3, stride=1, dropout_rate=0.2):
    """
    Residual block with skip connection (same pattern as original script,
    but works with longer time dimension).
    """
    shortcut = x
...
def build_resnet1d(input_shape=(SIGNAL_LENGTH, N_LEADS), num_classes=TARGET_CLASSES):
    ...
    NOTE:
    - This is intentionally very similar to your original kaggle_train script
      so weights remain interpretable and transfer easily.
    """
    ...
    # Residual blocks (same pattern as before)
...
# Calculate class weights for imbalanced data (keep same strategy)
class_weights = {}
...
print("\n" + "=" * 70)
print("🎯 STEP 5: Compiling and Training Model (500Hz)")
print("=" * 70)

model.compile(
    optimizer=keras.optimizers.Adam(learning_rate=LEARNING_RATE),
    loss="categorical_crossentropy",  # Phase 1: keep CE; Focal Loss in a later phase
    metrics=[
        "accuracy",
...
print("\n" + "=" * 70)
print("🎉 TRAINING COMPLETE! (Phase 1: 500Hz)")
print("=" * 70)
...
print("\n📚 Next Steps:")
print("   1. Integrate model.weights.h5 and classes.npy into backend-api/models/")
print("   2. Update backend classifier to expect (5000, 12) signals")
print("   3. Verify API predictions and BPM calculations at 500Hz\n")